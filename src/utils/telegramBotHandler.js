...
const startBot = () => {
  if (!TELEGRAM_BOT_TOKEN) {
    console.error('Telegram bot token not configured. Bot polling not started.');
    return;
  }
  ...
  if (text.startsWith('/')) {
    ...
    switch (command) {
      ...
      case '/help':
        ...
        break;
      default:
        await telegramService.sendMessage('Unknown command. Type /help for available commands.', chatId);
        break;
    }
  }
};
...
const handleConnectCommand = async (chatId, code, message) => {
  if (!code) {
    await telegramService.sendMessage('Please provide a connection code. Example: /connect 123456', chatId);
    return;
  }
  ...
  if (!result.success) {
    await telegramService.sendMessage(`❌ ${result.error}. Please try again with a valid code.`, chatId);
    return;
  }
...
To test the connection, go to the Notification Settings page in the web app.
    ...
    await telegramService.sendMessage('❌ An error occurred while connecting your account. Please try again later.', chatId);
  }
};
...
const setupWebhook = async () => {
  ...
  try {
    ...
    if (response.data.ok) {
      ...
      console.error('Error setting up Telegram webhook:', response.data.description);
    }
  } catch (error) {
