...
async function main() {
  ...
  console.log("Deploying Chain Shield contracts...");
  console.log(
    `Network: ${hre.network.name} (Chain RPC: ${hre.network.config.url}) (Chain ID: ${hre.network.config.chainId})`
  );

  // Get the contract factories
  const TransactionMonitor = await hre.ethers.getContractFactory(
    "TransactionMonitor"
  );
  const RugPullDetector = await hre.ethers.getContractFactory(
    "RugPullDetector"
  );
  const WalletDrainerDetector = await hre.ethers.getContractFactory(
    "WalletDrainerDetector"
  );
  const SWatchdogRegistry = await hre.ethers.getContractFactory(
    "SWatchdogRegistry"
  );
  ...
  console.log(
    `TransactionMonitor deployed to: ${await transactionMonitor.getAddress()}`
  );
  ...
  console.log(
    `RugPullDetector deployed to: ${await rugPullDetector.getAddress()}`
  );
  ...
  console.log(
    `WalletDrainerDetector deployed to: ${await walletDrainerDetector.getAddress()}`
  );
