...
async function main() {
  try {
    ...
    const transactionMonitor = await transactionMonitorFactory.deploy(
    ...
    const rugPullDetector = await rugPullDetectorFactory.deploy(
      rugPullThreshold
    );
    ...
    const walletDrainerDetectorAddress =
      await walletDrainerDetector.getAddress();
    console.log(
      `WalletDrainerDetector deployed to: ${walletDrainerDetectorAddress}`
    );
